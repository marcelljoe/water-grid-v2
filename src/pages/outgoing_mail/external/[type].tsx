...
import { TimelineConnector, Timeline, TimelineContent, TimelineDot, TimelineSeparator } from '@mui/lab';
import TimelineItem, { timelineItemClasses } from '@mui/lab/TimelineItem';

import withAuth from '@/hocs/withAuth';
import Switch from '@/components/FormControlled/Formik/Switch';
import { Field, FormikProvider, useFormik, Form as FormikForm } from 'formik';
...
const DetailOutgoingMail: React.FC<DetailOutgoingMailProps> = props => {
  ...
  const formikPassword = useFormik({
    ...
    onSubmit: async values => {
      try {
        ...
        const response = await http.post(`/api/mail/mail_approval`, body);
        actions.UPDATE_DIALOG({
          ...state.dialog,
          show: true,
          image: '/images/success.png',
  ...
  const handleRedirect = () => {
    ...
    router.push('/outgoing_mail/external');
  };
  ...
  return (
    <React.Fragment>
      <Head>
        <title>Detail Surat Keluar Eksternal</title>
      </Head>
      <LayoutComponent>
        <>
          <Stack className="main-title-row" direction="row" justifyContent="space-between">
            <Box className="head-moment-sub">
              <h2 title="TEST" className="align-month main-title">
                Detail Surat Keluar - Eksternal
              </h2>
            </Box>
            <Stack direction="row" spacing={1}>
              {props.mailDetail?.editable && (
                <Link
                  href={'/outgoing_mail/edit?type=2&uuid=' + props.mailDetail?.uuid + `&d=${props.mailDetail.isDraft}`}
                >
              ...
              {props.mailDetail.isSend && (
                <Button className="btn-secondary" fullWidth variant="contained">
                  Kirim
                </Button>
              )}
            </Stack>
            ...
            <Grid item xs={12} sm={8}>
              ...
              >
                ...
                <Stack className="custom-mail" justifyContent="center" alignItems="center">
                  <PrintPdf mailDetail={props.mailDetail} type="2" />
                </Stack>
              </Paper>
            </Grid>
              ...
              <FormikForm onSubmit={onSubmitForm}>
                ...
                <Stack direction="column" sx={{ padding: '0 25px' }} justifyContent="center" alignItems="center">
                  <img
                    ...
                    height={320}
                  />
                  <p style={{ fontSize: '12px', color: '#536580', marginBottom: '25px', textAlign: 'center' }}></p>
                  <Switch
                    value={checked}
                  ...
                  />
                  {checked && (
                    <form method="post" style={{ marginTop: '15px', width: '100%' }}>
                      <Field
                        style={{ width: '100%' }}
                        component={TextField}
...
export const getServerSideProps: GetServerSideProps = async ctx => {
  ...
  const history = await mailService.getHistory(session?.session as string, data.id, 2);
...
export default withAuth(DetailOutgoingMail, '/outgoing_mail/external');
