...
const IncomingMail = () => {
  ...
  const { data: data, error } = useSWR(
    `/api/mail/outbox_external?rowPerPage=${state.pagination.rowPerPage}&column=${state.filter.column}&direction=${state.filter.direction}&page=${state.pagination.currentPage}&key=${state.filter.key}&status=${state.filter.status}&startDate=${state.filter.startDate}&endDate=${state.filter.endDate}&statusLetter=${state.filter.statusLetter}`,
    fetcher
  );
  ...
  return (
    <>
      <React.Fragment>
        <Head>
          <title>Surat Keluar Eksternal | E-Office Web Apps</title>
        </Head>
        <>
          <LayoutComponent>
            <>
              <Stack className="main-title-row" direction="row" justifyContent="space-between">
                ...
                <Stack direction="row" spacing={1}>
                  ...
                  <Link href="/outgoing_mail/create?type=2">
